 ...
 * Error taxonomy.
 *
 * Every failure path in otplink throws an `OtpLinkError` carrying a stable
...
export type OtpLinkErrorCode =
    ...
    | "delivery_failed"
    /** Programmer error: bad config passed to `createOtpLink`. */
    | "configuration_error";

const PUBLIC_MESSAGES: Record<OtpLinkErrorCode, string> = {
...
const STATUS: Record<OtpLinkErrorCode, number> = {
    invalid_email: 400,
    rate_limited: 429,
...
export interface OtpLinkErrorOptions {
...
export class OtpLinkError extends Error {
    readonly code: OtpLinkErrorCode;
    ...
    constructor(code: OtpLinkErrorCode, message?: string, options: OtpLinkErrorOptions = {}) {
        super(message ?? code, options.cause !== undefined ? { cause: options.cause } : undefined);
        this.name = "OtpLinkError";
        this.code = code;
    ...
    static is(value: unknown): value is OtpLinkError {
        return value instanceof Error && value.name === "OtpLinkError" && "code" in value;
    }

    /** Shape suitable for a JSON error response. Never includes `message`. */
    toJSON(): { error: OtpLinkErrorCode; message: string; retryAfter?: number } {
        return {
            error: this.code,
